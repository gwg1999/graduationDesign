        ...
        <template slot-scope="scope">
          <el-form>
            <div style="display: flex;justify-content: space-evenly;font-size: 4px;height: 40px">
              ...
              <el-form-item>
                供货周期:<el-input-number :min="0" @keyup.119.native="searchNoCustomerList(scope.row.wId)" @keyup.118.native="searchWhole(scope.row.wId)" @keyup.117.native="searchHistoryList(scope.row.wId)" @keyup.116.native="searchList(scope.row.wId)"  v-model = "scope.row.indDeliveryCycle"  size="small"></el-input-number>
              </el-form-item>
...
export default {
  ...
  methods: {
    ...
    //零件售卖记录该客户
    searchList(pId){
      let customerId=this.$route.query.inPrice.indCustomerId
    ...
    submitForm(){
      if(this.salesSlip.quotationDetailList.length===0){
        ...
        this.$refs['salesSlip'].validate((valid) => {
          if (valid) {
            this.salesSlip.iNote=this.inPrice.iNote
            ...
            PostData('/inquiry/addInquiry',this.submitInfo)
              .then(res=>{
                this.$message({
                  type:'success',
                  message:'添加成功'
    ...
    assignment(){
      ...
      this.submitInfo.iprice=this.salesSlip.qPrice
      this.submitInfo.iNote=this.inPrice.iNote
      this.submitInfo.inquiryDetailList.splice(0,this.submitInfo.inquiryDetailList.length)
      this.submitInfo.inquiryDetailList.push(...this.salesSlip.quotationDetailList)
      this.submitInfo.inquiryDetailList.push(...this.salesSlip.wholeDetailsList)
    },
    //添加零件
    addPart(item){
    ...
    queryGoods(pageNum=1){
      if (this.levelIV.qdType === 1) {
        ...
        levelIVCopy.pFactoryId=this.inPrice.factoryId
