...
<!--            </el-form-item>-->
    ...
    <el-button type="primary" :disabled='isEnable' @click="submitForm('car')">确 定</el-button>
  </span>
    </el-dialog>
    <!-- 表格 -->
    <el-table
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          {{ (carQuery.pageNum - 1) * carQuery.pageSize + scope.$index + 1 }}
        </template>
      </el-table-column>
<!--      <el-table-column prop="cBrandId" label="车品牌ID" width="100%"  align="center"/>-->
      <el-table-column prop="cBrandName" label="车品牌名" width="300%"  align="center"/>
...
<!--      <el-table-column prop="cuName" label="联系人姓名" width="100%" align="center" />-->
    ...
    <el-pagination
      ...
      :page-size="carQuery.pageSize"
      :current-page="carQuery.pageNum"
      :total="pageTotal"
...
export default {
  ...
  methods:{//创建具体的方法
    ...
    toInsert(){
      this.$router.push('/maintain/car/addCar')
    },
    getListByLike(){
      this.carQuery.cTypeName=this.state
      this.carQuery.pageNum=1
      PostData('/car/selectAllByLike',qs.stringify(this.carQuery))
        .then(res=>{
          this.list=res.list
          this.pageTotal=res.total
    ...
    getPageTotal(){
      this.selectCondition = JSON.parse(JSON.stringify(this.carQuery))
      this.selectCondition.pageNum = this.selectCondition.pageSize = 0
      PostData('/car/selectAllByLike',this.selectCondition).then(res=>{
    ...
    //跳转详情页
    showDetails(index){
      localStorage.setItem('cId',this.list[index].cId)
    ...
    querySearch(queryString, cb) {
      this.carQuery.cTypeName=queryString;
      this.carQuery.pageNum=1
      PostData('car/selectAllByLike',qs.stringify(this.carQuery)).then(ref=>{
        cb(ref.list)
      })
      // 调用 callback 返回建议列表的数据
    },
    handleSelect(item) {
      this.state=item.cTypeName
    },
    enable(){
      setTimeout(()=>{
        this.isEnable=false
